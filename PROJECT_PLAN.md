...
**FAILED-state defect, found by reading rather than by any test.** The first version of the strip computed `stageIndex("failed")`, which is `-1` because `failed` is not a position in `STAGES` - so a failed job painted **all seven pills red, including `completed`**, asserting a stage that never ran. `vite build` passed on it. Fixed by recording `failedAt` (the stage that was running when the failure landed) in `applyJobEvent` and deriving the strip in `stageStates()`, inside the module the driver covers. Proven against a **real** fault-injected failure, using Phase 10's recipe - a second server on :4001 with `S3_BUCKET` pointing at a bucket that does not exist:

```
JOB - Failed
[x]received [x]validating [x]resolving [x]compiling [x]generating [!]uploading [ ]completed
Failed at uploading: Upload failed for "circuit-diagram.svg": The specified bucket does not exist
...
- [x] Dev UI (`web/`) — reconnected to the real backend (done — listened for a `job:status` event the server never emits, so all five per-stage events went unheard; socket handlers never called `setJob`, so the panel was frozen at the POST response; `compilable`/`mockedPinCount` were never rendered, so a board with positional pins read as a clean success; and a failed job painted all seven stage pills red including `completed`. All four fixed and measured live, both terminal paths, the FAILED one against a real fault-injected server. LIMITATION, accepted not hidden: the panel's LOGIC is verified live against `web/src/jobState.js`, the component's own state module — the RENDER is not. No browser automation is installed and none was added, so the JSX is covered only by `vite build` passing plus a class-name-vs-stylesheet audit; layout/overflow/contrast are unverified and no human has looked at the page.)
